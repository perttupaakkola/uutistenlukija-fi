---
title: "STTK: keskipalkkaisten ostovoima ei ole toipunut"
description: "Keski- ja pienipalkkaisten ostovoima on edelleen heikolla tasolla."
image: "https://images.unsplash.com/photo-1632385820055-8370ff5a17a2?crop=entropy&cs=srgb&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHxidXNpbmVzcyUyMGZpbmFuY2UlMjBlY29ub215fGVufDF8MHx8fDE3NzQwMDE0ODd8MA&ixlib=rb-4.1.0&q=85"
image_alt: "STTK: keskipalkkaisten ostovoima ei ole toipunut"
image_credit: "Photo by CHUTTERSNAP on Unsplash"
image_source_url: "https://unsplash.com/photos/a-large-body-of-water-with-a-city-in-the-background-8JKDRFhEWME?utm_source=uutistenlukija&utm_medium=referral"
image_thumb: "https://images.unsplash.com/photo-1632385820055-8370ff5a17a2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHxidXNpbmVzcyUyMGZpbmFuY2UlMjBlY29ub215fGVufDF8MHx8fDE3NzQwMDE0ODd8MA&ixlib=rb-4.1.0&q=80&w=400"
date: 2026-03-17T06:17:47.042300+00:00
categories:
  - Talous
author: "Mikko Salonen"
author_id: "mikko-salonen"
author_title: "Kulttuuritoimittaja"
author_bio: "Kulttuurin ja viihteen monitoimittaja. Seuraa musiikkia, elokuvia, kirjallisuutta ja taidemaailmaa."
author_image: "/images/writers/mikko-salonen.jpg"
draft: false
---

Keski- ja pienipalkkaisten ostovoima on edelleen heikolla tasolla. STTK:n ostovoimakatsaus osoittaa, että inflaation ja verotuksen yhteisvaikutus on syönyt palkansaajien ostovoimaa viime vuosina.

STTK esittää ratkaisuksi maltillista palkkaverotuksen keventämistä. Järjestön mukaan kevennykset tulisi kohdistaa keski- ja pienituloisille, joiden arki on kiristynyt eniten.

Ostovoiman heikkeneminen näkyy kotitalouksien kulutuskäyttäytymisessä. Monet perheet ovat joutuneet karsimaan menojaan, ja kulutuksen kasvu on hidastunut. Tilanne jarruttaa samalla kotimaista talouskehitystä.

Suomessa on käyty keskustelua veropolitiikan suunnasta. STTK:n katsaus tuo keskusteluun lukuja siitä, miten eri tuloryhmien asema on kehittynyt. Palkansaajajärjestö painottaa, että veronkevennykset eivät yksin riitä, vaan niiden rinnalla tarvitaan vastuullista palkkapolitiikkaa.
