---
title: "Nvidia tähtää tekoälyn seuraavaan taistelukenttään koulutusvaiheesta käyttövaiheeseen"
description: "Nvidia on hallinnut tekoälymallien kouluttamiseen käytettävien sirujen markkinoita jo vuosia."
image: "https://images.unsplash.com/photo-1573757056004-065ad36e2cf4?crop=entropy&cs=srgb&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHx0ZWNobm9sb2d5JTIwaW5ub3ZhdGlvbiUyMGRpZ2l0YWx8ZW58MXwwfHx8MTc3Mzk2OTA4OXww&ixlib=rb-4.1.0&q=85"
image_alt: "Nvidia tähtää tekoälyn seuraavaan taistelukenttään koulutusvaiheesta käyttövaiheeseen"
image_credit: "Photo by Jonas Morgner on Unsplash"
image_source_url: "https://unsplash.com/photos/man-standing-while-using-computer-and-another-group-of-men-chatting-F7u5fL11Lt0?utm_source=uutistenlukija&utm_medium=referral"
image_thumb: "https://images.unsplash.com/photo-1573757056004-065ad36e2cf4?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHx0ZWNobm9sb2d5JTIwaW5ub3ZhdGlvbiUyMGRpZ2l0YWx8ZW58MXwwfHx8MTc3Mzk2OTA4OXww&ixlib=rb-4.1.0&q=80&w=400"
date: 2026-03-17T06:17:47.042300+00:00
categories:
  - Teknologia
author: "Anna Korhonen"
author_id: "anna-korhonen"
author_title: "Taloustoimittaja"
author_bio: "Talouden ja markkinoiden asiantuntija. Kirjoittanut laajasti Suomen ja EU:n talouspolitiikasta sekä teknologiainvestoinneista."
author_image: "/images/writers/anna-korhonen.jpg"
draft: false
tags:
  - teknologia
  - yritys
  - koulutus
  - tekoäly
  - urheilu
related_articles:
  - 2026-03-16-nvidian-vuosikonferenssi-alkaa-toimitusjohtaja-esittelee-tek
  - 2026-03-17-nvidia-julkisti-nemoclaw-alustan-tekoalyagenttien-rakentamis
  - 2026-03-17-nvidia-julkaisi-avoimen-tekoalyagenttialustan-yrityksia-vart
---

Nvidia on hallinnut tekoälymallien kouluttamiseen käytettävien sirujen markkinoita jo vuosia. Nyt yhtiö suuntaa katseensa seuraavaan vaiheeseen: tekoälyn pyörittämiseen tuotantokäytössä. Kilpailu kiihtyy, kun yhä useampi yritys ottaa tekoälysovelluksia käyttöön.

Nvidian toimitusjohtaja Jensen Huang esitteli yhtiön suunnitelmia GTC-konferenssissa. Huang ennusti tekoälyinfrastruktuurin kasvavan voimakkaasti lähivuosina, kun mallien käyttö siirtyy koulutuksesta laajamittaiseen tuotantoon.

Koulutusvaiheen siruissa Nvidian asema on ollut ylivoimainen. GPU-sirut ovat käytännössä välttämättömiä suurten kielimallien rakentamisessa. Käyttövaiheen sirut ovat kuitenkin eri markkina, jossa kilpailijoita on enemmän. Amazon, Google ja muut teknologiayhtiöt kehittävät omia sirujaan juuri tähän tarkoitukseen.

Nvidian osake on noussut tekoälybuumin myötä voimakkaasti, ja yhtiön markkina-arvo on kasvanut maailman suurimpien joukkoon.
