---
title: "Mies raiskasi koulumatkalla olleen teinitytön Helsingin Kivikossa"
description: "19-vuotias Elias Mahad Aden raiskasi 16-vuotiaan tytön Helsingin Kivikossa lokakuussa 2023."
image: "https://images.unsplash.com/photo-1674931009515-2a9348f4b1cb?crop=entropy&cs=srgb&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHxmaW5sYW5kJTIwbGFuZHNjYXBlJTIwY2l0eXxlbnwxfDB8fHwxNzczOTUyNTE1fDA&ixlib=rb-4.1.0&q=85"
image_alt: "Mies raiskasi koulumatkalla olleen teinitytön Helsingin Kivikossa"
image_credit: "Photo by K8 on Unsplash"
image_source_url: "https://unsplash.com/photos/a-large-body-of-water-with-a-bridge-in-the-background-e7ChKEttaEw?utm_source=uutistenlukija&utm_medium=referral"
image_thumb: "https://images.unsplash.com/photo-1674931009515-2a9348f4b1cb?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHxmaW5sYW5kJTIwbGFuZHNjYXBlJTIwY2l0eXxlbnwxfDB8fHwxNzczOTUyNTE1fDA&ixlib=rb-4.1.0&q=80&w=400"
date: 2026-03-17T00:20:56.278194+00:00
categories:
  - Kotimaa
author: "Sanna Heikkinen"
author_id: "sanna-heikkinen"
author_title: "Ulkomaantoimittaja"
author_bio: "Ulkomaantoimittaja, jolla on laaja kokemus Euroopan ja Lähi-idän raportoinnista. Aiemmin kirjeenvaihtajana Brysselissä."
author_image: "/images/writers/sanna-heikkinen.jpg"
draft: false
tags:
  - kotimaa
  - koulutus
  - luonto
  - sää
  - helsinki
---

19-vuotias Elias Mahad Aden raiskasi 16-vuotiaan tytön Helsingin Kivikossa lokakuussa 2023. Aden lähestyi uhriaan bussipysäkillä tytön ollessa matkalla kouluun 5. lokakuuta.

Aden aloitti keskustelun tytön kanssa ja käänsi sen nopeasti seksuaaliseksi. Hän talutti tyttöä kädestä pitäen läheiseen metsään, jossa raiskasi tämän törkeällä tavalla.

Tekoa käsitellään törkeänä raiskauksena. Uhri oli tekohetkellä alaikäinen ja tekijä tuntematon aikuinen mies, joka lähestyi tyttöä julkisella paikalla keskellä päivää.

Tapaus on edennyt oikeuteen. Tekijän henkilöllisyys on julkinen rikoksen vakavuuden vuoksi.
