---
title: "Pohjois-Koreassa järjestettiin vaalit – äänestysaktiivisuus lähes sata prosenttia"
date: 2026-03-16T00:15:16.939899+00:00
categories:
  - Ulkomaat
author: "Anna Korhonen"
author_id: "anna-korhonen"
author_title: "Taloustoimittaja"
author_bio: "Talouden ja markkinoiden asiantuntija. Kirjoittanut laajasti Suomen ja EU:n talouspolitiikasta sekä teknologiainvestoinneista."
author_image: "/images/writers/anna-korhonen.jpg"
draft: false
tags:
  - ulkomaat
  - politiikka
  - kansainvälinen
related_articles:
  - 2026-03-16-ranskan-paikallisvaalit-kiinnostavat-presidentinvaalien-ennu
  - 2026-03-18-hatatila-teneriffalla-sotki-suomalaisen-loman-retket-peruttu
  - 2026-03-19-kuningas-avasi-maailman-pisimman-rannikkopatkan-englannissa
description: "Pohjois-Koreassa järjestettiin kansallisen kansankokouksen vaalit, joissa äänestysaktiivisuus nousi lähes sataan prosenttiin, viranomaisten mukaan."
image: "https://images.unsplash.com/photo-1766964288691-9a3f8ac1be35?crop=entropy&cs=srgb&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHx3b3JsZCUyMGdsb2JlJTIwaW50ZXJuYXRpb25hbHxlbnwxfDB8fHwxNzczOTUyNTEyfDA&ixlib=rb-4.1.0&q=85"
image_alt: "Pohjois-Koreassa järjestettiin vaalit – äänestysaktiivisuus lähes sata prosenttia"
image_credit: "Photo by Vito Natale on Unsplash"
image_source_url: "https://unsplash.com/photos/a-small-globe-on-a-wooden-table-kKgZh-KYpw8?utm_source=uutistenlukija&utm_medium=referral"
image_thumb: "https://images.unsplash.com/photo-1766964288691-9a3f8ac1be35?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHx3b3JsZCUyMGdsb2JlJTIwaW50ZXJuYXRpb25hbHxlbnwxfDB8fHwxNzczOTUyNTEyfDA&ixlib=rb-4.1.0&q=80&w=400"
---

Pohjois-Korea järjesti sunnuntaina kansallisen kansankokouksen vaalit. Maan viranomaisten mukaan äänestysaktiivisuus nousi lähes sataan prosenttiin, kuten maassa on tapana.

Kansankokousta pidetään käytännössä kumileimasimena, joka hyväksyy Korean työväenpuolueen päätökset sellaisinaan. Ehdokaslistoilla on tyypillisesti vain yksi nimi vaalipiiriä kohden, joten varsinaisesta vaalista ei ole kyse sanan länsimaisessa merkityksessä. Äänestämättä jättäminen tulkitaan käytännössä epälojaaliudeksi valtiota kohtaan.

Vaalit järjestettiin pian helmikuisen puoluekokouksen jälkeen. Puoluekokouksessa Korean työväenpuolue linjasi politiikkaansa ja teki henkilövalintoja, joita kansankokouksen odotetaan nyt vahvistavan.

Pohjois-Korean vaalijärjestelmä poikkeaa demokraattisten maiden käytännöistä. Äänestys toimii pikemminkin väestönlaskennan ja lojaliteetin mittaamisen välineenä. Kansainväliset tarkkailijat eivät seuraa maan vaaleja.
