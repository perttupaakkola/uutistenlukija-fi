---
title: "Tutkijat löysivät uuden planeettatyypin, jota kuvataan helvetilliseksi"
description: "Astronomit ovat tunnistaneet aiemmin tuntemattoman planeettatyypin."
image: "https://images.unsplash.com/photo-1618053448748-b7251851d014?crop=entropy&cs=srgb&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHxzY2llbmNlJTIwcmVzZWFyY2glMjBsYWJvcmF0b3J5fGVufDF8MHx8fDE3NzQwMDIwODF8MA&ixlib=rb-4.1.0&q=85"
image_alt: "Tutkijat löysivät uuden planeettatyypin, jota kuvataan helvetilliseksi"
image_credit: "Photo by DIANA HAUAN on Unsplash"
image_source_url: "https://unsplash.com/photos/woman-in-white-laboratory-gown-holding-black-microphone-oFK7xgf4_50?utm_source=uutistenlukija&utm_medium=referral"
image_thumb: "https://images.unsplash.com/photo-1618053448748-b7251851d014?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHxzY2llbmNlJTIwcmVzZWFyY2glMjBsYWJvcmF0b3J5fGVufDF8MHx8fDE3NzQwMDIwODF8MA&ixlib=rb-4.1.0&q=80&w=400"
date: 2026-03-17T03:10:19.053743+00:00
categories:
  - Tiede
author: "Sanna Heikkinen"
author_id: "sanna-heikkinen"
author_title: "Ulkomaantoimittaja"
author_bio: "Ulkomaantoimittaja, jolla on laaja kokemus Euroopan ja Lähi-idän raportoinnista. Aiemmin kirjeenvaihtajana Brysselissä."
author_image: "/images/writers/sanna-heikkinen.jpg"
draft: false
tags:
  - tiede
  - avaruus
  - sää
---

Astronomit ovat tunnistaneet aiemmin tuntemattoman planeettatyypin. Löydettyjen planeettojen olosuhteet ovat niin äärevät, että tutkijat kutsuvat niitä helvetillisiksi.

Uusi planeettaluokka erottuu tunnetuista tyypeistä pintalämpötilojensa ja ilmakehänsä koostumuksen perusteella. Planeetat kiertävät emätähtiään lähellä, mikä altistaa ne voimakkaalle säteilylle ja nostaa pintalämpötilan äärimmäiseksi. Olosuhteet tekevät niistä elinkelvottomia minkään tunnetun elämänmuodon kannalta.

Löytö saattaa vaikuttaa arvioihin siitä, kuinka todennäköistä elämän syntyminen on Maan ulkopuolella. Jos tämän tyyppisiä planeettoja on runsaasti, osa aiemmin elinkelpoisiksi arvioiduista planeetoista voi todellisuudessa kuulua tähän helvetilliseen luokkaan.

Planeettoja on löydetty tuhansia viime vuosikymmeninä, ja luokittelujärjestelmää on päivitetty sitä mukaa kun havaintoteknologia on kehittynyt. Uusi tyyppi osoittaa, että planeettakunta on monimuotoisempi kuin aiemmat mallit ennustivat. Tutkijat jatkavat havaintoja tarkentaakseen planeettatyypin yleisyyttä galaksissamme.
