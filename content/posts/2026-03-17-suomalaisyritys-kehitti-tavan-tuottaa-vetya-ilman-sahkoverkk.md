---
title: "Suomalaisyritys kehitti tavan tuottaa vetyä ilman sähköverkkoa"
description: "Suomalainen Zun-H on kehittänyt järjestelmän, joka tuottaa uusiutuvaa vetyä ilman sähköverkkoa tai ulkoista sähkönsyöttöä."
image: "https://images.unsplash.com/photo-1573757056004-065ad36e2cf4?crop=entropy&cs=srgb&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHx0ZWNobm9sb2d5JTIwaW5ub3ZhdGlvbiUyMGRpZ2l0YWx8ZW58MXwwfHx8MTc3Mzk2OTA4OXww&ixlib=rb-4.1.0&q=85"
image_alt: "Suomalaisyritys kehitti tavan tuottaa vetyä ilman sähköverkkoa"
image_credit: "Photo by Jonas Morgner on Unsplash"
image_source_url: "https://unsplash.com/photos/man-standing-while-using-computer-and-another-group-of-men-chatting-F7u5fL11Lt0?utm_source=uutistenlukija&utm_medium=referral"
image_thumb: "https://images.unsplash.com/photo-1573757056004-065ad36e2cf4?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5MDAzMjV8MHwxfHNlYXJjaHwxfHx0ZWNobm9sb2d5JTIwaW5ub3ZhdGlvbiUyMGRpZ2l0YWx8ZW58MXwwfHx8MTc3Mzk2OTA4OXww&ixlib=rb-4.1.0&q=80&w=400"
date: 2026-03-17T03:10:19.053743+00:00
categories:
  - Teknologia
author: "Sanna Heikkinen"
author_id: "sanna-heikkinen"
author_title: "Ulkomaantoimittaja"
author_bio: "Ulkomaantoimittaja, jolla on laaja kokemus Euroopan ja Lähi-idän raportoinnista. Aiemmin kirjeenvaihtajana Brysselissä."
author_image: "/images/writers/sanna-heikkinen.jpg"
draft: false
tags:
  - teknologia
  - energia
  - yritys
  - liikenne
---

Suomalainen Zun-H on kehittänyt järjestelmän, joka tuottaa uusiutuvaa vetyä ilman sähköverkkoa tai ulkoista sähkönsyöttöä. Teknologia perustuu näkyvän valon hyödyntämiseen.

Järjestelmässä valo virittää elektroneja tietyissä materiaaleissa, mikä käynnistää kemiallisen reaktion ja tuottaa vetyä. Perinteisessä elektrolyysissä vety erotetaan vedestä sähkövirran avulla, mutta Zun-H:n ratkaisu ohittaa tämän vaiheen.

Yhtiön tavoite on haastaa nykyinen energiantuotannon malli. Jos teknologia skaalautuu teolliseen mittakaavaan, se voisi mahdollistaa vedyntuotannon paikoissa, joissa sähköverkkoa ei ole tai sen rakentaminen olisi kallista.

Vedyn tuotanto on toistaiseksi nojannut pitkälti maakaasun reformointiin tai sähköä vaativaan elektrolyysiin. Zun-H:n valokemiallisen menetelmän kaupallistamisaikataulu ei ole vielä tiedossa.
