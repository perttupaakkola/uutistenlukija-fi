...
- Keep deterministic staged pipeline workflows independently marker-gated. Scanner canaries prefer authenticated `workflow_dispatch` from exact `refs/heads/main`. If dispatch permission is unavailable, `pipeline/actions-scan.enabled` may be added for exactly one scheduled canary and must be removed after that run completes, before the next boundary; otherwise the marker is reserved for final scheduled cutover. Until the historically exposed Firehose credential is rotated and separately reviewed, the scanner is intentionally RSS/public-research only and must not inject `FIREHOSE_TOKEN`. Publisher control uses `pipeline/actions-publish.enabled`. Changing or rolling back one must not toggle the other.
- Host-authored staged outbox pushes may admit only the existing marker-gated, max-1 publisher path; publisher commits use `GITHUB_TOKEN` and must not recurse.
