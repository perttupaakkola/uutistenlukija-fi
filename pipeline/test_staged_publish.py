...
class StagedPublishMetricsTests(unittest.TestCase):
    ...
    def test_preflight_reject_terminalizes_with_full_evidence_and_exact_queue_delta(self) -> None:
        reject_path, reject_data, _ = self._write_publish_record("20260901T120000Z_preflight-reject")
        review_path, review_data, _ = self._write_publish_record("20260901T120100Z_preflight-review")
        review_before = review_path.read_bytes()
        args = Namespace(max_articles=1, dedup_window=48, dry_run=False, git_push=True)

        def preflight(data):
            packet_id = data["packet"]["packet_id"]
            action = "reject" if packet_id.endswith("preflight-reject") else "monica_review"
            reasons = ("category_disagreement",) if action == "reject" else ("thin_distinct_source",)
            return type(
                "Preflight",
                (),
                {
                    "action": action,
                    "requires_monica_review": action == "monica_review",
                    "reasons": reasons,
                    "categories": ("Kotimaa", "Talous", "Kotimaa") if action == "reject" else ("Kotimaa",) * 3,
                    "selected_source_urls": ("https://example.test/source",),
                    "public_source_urls": ("https://example.test/source",),
                    "hidden_source_urls": (),
                    "distinct_source_words": 260,
                    "article_words": 260,
                    "article_source_ratio": 1.0,
                    "sensitive": False,
                },
            )()

        with patch.object(staged_publish, "load_outbox", return_value=[(reject_path, reject_data), (review_path, review_data)]), \
             patch.object(staged_publish, "evaluate_publish_preflight", side_effect=preflight), \
             patch.object(staged_publish, "persist_queue_transitions", return_value=0) as persist:
            rc = staged_publish.cmd_publish(args)

        self.assertEqual(rc, 0)
        failed_path = self.root / "failed" / reject_path.name
        self.assertFalse(reject_path.exists())
        self.assertTrue(failed_path.exists())
        failed = json.loads(failed_path.read_text(encoding="utf-8"))
        self.assertTrue(failed["publish_preflight_rejected"])
        self.assertTrue(failed["publish_preflight_rejected_at"].endswith("+00:00"))
        self.assertEqual(
            failed["publish_preflight_feedback"],
            {
                "action": "reject",
                "article_source_ratio": 1.0,
                "article_words": 260,
                "categories": ["Kotimaa", "Talous", "Kotimaa"],
                "distinct_source_words": 260,
                "hidden_source_urls": [],
                "public_source_urls": ["https://example.test/source"],
                "reasons": ["category_disagreement"],
                "requires_monica_review": False,
                "selected_source_urls": ["https://example.test/source"],
                "sensitive": False,
            },
        )
        self.assertEqual(failed["failure"], "publish_preflight_rejected: category_disagreement")
        self.assertTrue(review_path.exists())
        self.assertEqual(review_path.read_bytes(), review_before)
        persist.assert_called_once_with([(reject_path, failed_path)])

    def test_preflight_reject_and_publish_processes_publish_but_does_not_push(self) -> None:
        reject_path, reject_data, _ = self._write_publish_record("20260901T121000Z_preflight-reject")
        publish_path, publish_data, publish_article = self._write_publish_record("20260901T121100Z_preflight-publish")
        args = Namespace(max_articles=1, dedup_window=48, dry_run=False, git_push=False)

        def preflight(data):
            action = "reject" if data is reject_data else "publish"
            return type(
                "Preflight",
                (),
                {
                    "action": action,
                    "requires_monica_review": False,
                    "reasons": ("category_disagreement",) if action == "reject" else (),
                    "categories": ("Kotimaa", "Talous", "Kotimaa") if action == "reject" else ("Kotimaa",) * 3,
        ...
        with patch.object(staged_publish, "load_outbox", return_value=[(reject_path, reject_data), (publish_path, publish_data)]), \
             patch.object(staged_publish, "evaluate_publish_preflight", side_effect=preflight), \
             patch.object(staged_publish, "run_quality_gate", return_value=type("Gate", (), {"passed": [publish_article], "rejected": []})()), \
             ...
             patch.object(staged_publish, "dedup_within_batch", side_effect=lambda articles: articles), \
             patch.object(staged_publish, "enrich_images_for_articles", return_value={"total": 1, "images": 1, "missing": 0}), \
             patch.object(staged_publish, "publish_articles", return_value=["content/posts/test.md"]), \
             patch.object(staged_publish, "build_site", return_value=(True, "")), \
             patch.object(staged_publish, "mark_published"), \
             patch.object(staged_publish, "persist_queue_transitions") as persist:
            rc = staged_publish.cmd_publish(args)

        self.assertEqual(rc, 0)
        self.assertTrue((self.root / "failed" / reject_path.name).exists())
        self.assertTrue((self.root / "published" / publish_path.name).exists())
        persist.assert_not_called()

    def test_preflight_reject_dry_run_preserves_outbox_bytes(self) -> None:
        reject_path, reject_data, _ = self._write_publish_record("20260901T122000Z_preflight-reject")
        before = reject_path.read_bytes()
        ...
        with patch.object(staged_publish, "load_outbox", return_value=[(reject_path, reject_data)]), \
             patch.object(staged_publish, "evaluate_publish_preflight", return_value=rejected), \
             patch.object(staged_publish, "persist_queue_transitions") as persist:
            rc = staged_publish.cmd_publish(args)

        self.assertEqual(rc, 0)
        self.assertTrue(reject_path.exists())
        self.assertEqual(reject_path.read_bytes(), before)
        self.assertFalse((self.root / "failed" / reject_path.name).exists())
        persist.assert_not_called()
