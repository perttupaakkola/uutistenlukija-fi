...
- Completed writer packets use the `source_usage_contract: v1` map: every selected URL has an explicit boolean `used` value and `dependent_claims`; unused URLs require an empty claim list. Collapse same-article URL aliases before source selection, and expose every used source through the shared structured `source_attributions` projection consumed by publisher and preflight.
