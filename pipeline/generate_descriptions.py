...
def call_openai(headline: str, lead_text: str) -> str:
    """Call OpenAI API to generate a meta description."""
...
def process_file(file_path: Path, api_key: str, dry_run: bool = False) -> bool:
    ...
    # Generate
    try:
        description = call_openai(headline, lead_text)
...
def generate_for_article_dict(article: dict, api_key: str = "") -> str | None:
    ...
    try:
        desc = call_openai(headline, lead_text)
...
def batch_mode(dry_run: bool = False):
    get_client()  # validate key up front
